...
%Desired absolute,relative pose trajectories
[xad,dxad,ddxad,xr_d,dxr_d,ddxr_d,grasp_data] = traj_gen(xa_in,xr_in,time);
...
while sim.simxGetConnectionId(clientID)~=-1
    ...
    data.f_ext(i,:) = psi_ext_data(i,1:3)';
...
pos_2_data = []; 
for j = 1:size(time,2)
    pos_1_data(j,:) = vec4(DQ(data.x1(:,j)).translation);
    pos_2_data(j,:) = vec4(DQ(data.x2(:,j)).translation);
    j = j+1; 
end

figure()
plot(tt,pos_1_data(:,2),'LineWidth',2);
hold on, grid on
plot(tt,pos_1_data(:,3),'LineWidth',2);
hold on,grid on
plot(tt,pos_1_data(:,4),'LineWidth',2);
xlabel('time [s]')
ylabel ('pos')
legend('x','y','z')
title('Panda1')
...
figure()
plot(tt,pos_2_data(:,2),'LineWidth',2);
hold on, grid on
plot(tt,pos_2_data(:,3),'LineWidth',2);
hold on,grid on
plot(tt,pos_2_data(:,4),'LineWidth',2);
xlabel('time [s]')
ylabel ('pos')
legend('x','y','z')
title('Panda2')