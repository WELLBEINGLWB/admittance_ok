...
%% Static simulation: grasped object with two arms.

include_namespace_dq;

%% Initialize variables
%%Admittance controller absolute pose
...
%%Admittance controller relative pose
...
psi_ext_a_data = zeros(size(time,2),6);
psi_ext_r_data = zeros(size(time,2),6);
...
data.tau2_send = []; data.tau2_read = []; data.f_ext = []; data.fa_ext = []; data.fr_ext = []; 
data.Ma = []; data.Mr = []; 
data.pos1 = []; data.pos2 = []; 
...
while sim.simxGetConnectionId(clientID)~=-1
    ...
    if i~=1
        ...
        %rel pose
        xr2_adm = vec8(xr_in);
        e_in2 = vec8(DQ(xr2_adm)'*DQ(xr_des));
        yr2_in = vec6(log(DQ(e_in2)));
        dyr2_in = zeros(6,1);
    end
    
    %External wrenches (world-frame)
    wr_1 = [0;0;0.5*weight;0.5*weight*l_obj;0;0];
    wr_2 = [0;0;0.5*weight;-0.5*weight*l_obj;0;0];

    data.f_ext(i,:) = -2*wr_1(1:3); 
    
    %linear mapping to cdts taskspace
    [Wa,Wr] = wrench_adapter(wr_1,wr_2,x1,x2,xa);
    ...
    %rel pose
    [xd2,dxd2,ddxd2,yr2,dyr2] = admittance_control(xr_des,dxr_des,ddxr_des,psi_ext_r',xr2_adm,yr2_in,dyr2_in,Md2,Kd2,Bd2,time);
...
figure()
plot(tt,data.fa_ext(:,1),'LineWidth',2);
hold on, grid on
plot(tt,data.fa_ext(:,2),'LineWidth',2);
hold on,grid on
plot(tt,data.fa_ext(:,3),'LineWidth',2);
xlabel('time [s]')
ylabel ('abs_force [N]')

figure()
plot(tt,data.fr_ext(:,1),'LineWidth',2);
hold on, grid on
plot(tt,data.fr_ext(:,2),'LineWidth',2);
hold on,grid on
plot(tt,data.fr_ext(:,3),'LineWidth',2);
xlabel('time [s]')
ylabel ('rel_force [N]')
