...
%% Full-dual position control: test robustness to variation of impedance gains.
%% Decreasing rate of stiffness and damping is computed in accordance with Ignatyev paper.
...
dx_data = zeros(size(time,2),1);
dy_data = zeros(size(time,2),1); 
dz_data = zeros(size(time,2),1); 
...
while sim.simxGetConnectionId(clientID)~=-1
    ...
    %Model external disturbance as constant force
    %External wrench
    wrench_ext = [0;0;-0.5;0;0;0]; 
    ...
    %store values
    data.kd(i,:) = [kx_data(i,:);ky_data(i,:);kz_data(i,:)];
    data.bd(i,:) = [dx_data(i,:);dy_data(i,:);dz_data(i,:)];
    
    %compute impedance matrices
    kt = diag([kx_data(i,:);ky_data(i,:);kz_data(i,:)]); 
    dt = diag([dx_data(i,:);dy_data(i,:);dz_data(i,:)]); 

    Kd_var = blkdiag(kt,kt);
    Bd_var = blkdiag(dt,dt); 

    %Compute compliant trajectory
    [xd,dxd,ddxd,yr,dyr] = admittance_control(xa_des,dxa_des,ddxa_des,psi_ext,xr_adm,yr_in,dyr_in,Md1,Kd_var,Bd_var,time);
...
%% Admittance gains
figure()
plot(tt,kx_data(:,1)','LineWidth',2);
hold on, grid on
plot(tt,ky_data(:,1)','LineWidth',2);
hold on,grid on
plot(tt,kz_data(:,1)','LineWidth',2);
legend('kx','ky','kz')
